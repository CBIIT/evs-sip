...
//const dataFilesDir = path.join(__dirname, "data_files", "PCDC");
...
const readPCDCMapping = () => {
    let content = fs
        .readFileSync(dataFilesDir + "/PCDC/pcdc-model-all.json")
        .toString();
    content = content.replace(/}{/g, ",");
    return JSON.parse(content);
};

const readNewPCDCMapping = () => {
  let content = fs
      .readFileSync(dataFilesDir + "/PCDC/new-pcdc-model-all.json")
      .toString();
  content = content.replace(/}{/g, ",");
  return JSON.parse(content);
};
...
const build_pcdc_node = (pcdc_mapping) => {

    let allNodes = {};

    for (let proj_name in pcdc_mapping) {
      for(let node_name in pcdc_mapping[proj_name]) {
        const node = pcdc_mapping[proj_name][node_name];

        let n = {};
        let id = proj_name + '/' + node_name;

        n.project = proj_name;
        n.node = node_name;
        n.ncit = node.n_n_code;
        n.ncit_PT = node.n_PT;
        //n.props = node.properties;
...
const build_pcdc_prop = (pcdc_mapping) => {
  ...
  for (let proj_name in pcdc_mapping) {
    for(let node_name in pcdc_mapping[proj_name]) {
      const node = pcdc_mapping[proj_name][node_name];
      const properties = pcdc_mapping[proj_name][node_name].properties;
      
      if(properties.length !== 0){
        properties.forEach(prop => {
          let p = {};
          let id = proj_name + '/' + node_name + '/' + prop.p_name;

          p.project = proj_name;
          p.node = node_name;
          p.node_pt = node.n_PT;
          p.prop = prop.p_name;
          p.ncit = prop.p_n_code;
          p.type = prop.p_type;
          p.values = prop.values;
...
const comparePCDCDictionary = () => {

    // const icdcdata = readICDCyamlFiles();
    // const icdcnewdata = readICDCyamlFilesNew();

    let pcdc_mapping = readPCDCMapping();
    let new_pcdc_mapping = readNewPCDCMapping();

    let resultNode = build_pcdc_node(pcdc_mapping);
    let resultNodeNew = build_pcdc_node(new_pcdc_mapping);

    let resultProp = build_pcdc_prop(pcdc_mapping);
    let resultPropNew = build_pcdc_prop(new_pcdc_mapping);

    const output_file_pcdc_node = path.join(__dirname, 'data_files', 'pcdc_node_mapping_comp.csv');

    const outputStream_node = fs.createWriteStream(output_file_pcdc_node, { flags: 'a' });

    outputStream_node.write("project, node, new_node, ncit\n");

    for (let key in resultNodeNew) {
        if(resultNode[key] !== undefined){
            outputStream_node.write(resultNodeNew[key].project + "," + resultNode[key].ncit_PT + "," + resultNodeNew[key].ncit_PT + "," + resultNodeNew[key].ncit + "\n");
        }else {
            outputStream_node.write(resultNodeNew[key].project + ",no match," + resultNodeNew[key].ncit_PT + "," + resultNodeNew[key].ncit + "\n");
        }
    } //end for loop in content

    for (let key in resultNode) {
        if(resultNodeNew[key] === undefined){
            outputStream_node.write(resultNode[key].project + "," + resultNode[key].ncit_PT + ",no match," + resultNode[key].ncit + "\n");
        }
    }
    ...
    const output_file_icdcprop = path.join(__dirname, 'data_files', 'pcdc_prop_mapping_comp.csv');

    const outputStream_prop = fs.createWriteStream(output_file_icdcprop, { flags: 'a' });

    outputStream_prop.write("project, node, property, new_property, ncit\n");

    for (let key in resultPropNew) {
        if(resultProp[key] !== undefined){
            outputStream_prop.write(resultPropNew[key].project + "," + resultPropNew[key].node_pt + "," + resultProp[key].prop + "," + resultPropNew[key].prop + "," + resultPropNew[key].ncit + "\n");
        } else {
            outputStream_prop.write(resultPropNew[key].project + "," + resultPropNew[key].node_pt + ",no match," + resultPropNew[key].prop + "," + resultPropNew[key].ncit + "\n");
        }
    } //end for loop in content

    for (let key in resultProp) {
        if(resultPropNew[key] === undefined){
            outputStream_prop.write(resultProp[key].project + "," + resultProp[key].node_pt + "," + resultProp[key].prop + ",no match," + resultProp[key].ncit + "\n");
        }
    }
    ...
    const output_file_icdc_value = path.join(__dirname, 'data_files', 'pcdc_value_mapping_comp.csv');
    ...
    for (let key in resultPropNew) {
        if(resultProp[key] !== undefined){
            if(resultPropNew[key].type === 'enum' && resultProp[key].type === 'enum'){
                ...
                if(JSON.stringify(resultPropNew[key].values) === JSON.stringify(resultProp[key].values)){

                 resultPropNew[key].values.forEach(newvalue => {
                    resultProp[key].values.forEach(oldvalue => {
                        if(newvalue.v_name === oldvalue.v_name){
                            outputStream_value.write(resultPropNew[key].project + "," + resultPropNew[key].node_pt + "," + resultPropNew[key].prop  + ",'" + oldvalue.v_name + "','" + newvalue.v_name + "'," +  newvalue.v_n_code + "\n");
                        }
                    });
                });
                    ...
                    resultPropNew[key].values.forEach(newvalue => {
                        ...
                        if(trigger === true){
                            outputStream_value.write(resultPropNew[key].project + "," + resultPropNew[key].node_pt + "," + resultPropNew[key].prop  + ",'" + oldval.v_name + "','" + newvalue.v_name + "'," + newvalue.v_n_code + "\n");
                            //console.log('match', resultPropNew[key].project + "," + resultPropNew[key].node_pt + "," + resultPropNew[key].prop  + "," + oldval.n + "," + newvalue.n + "," +  resultPropNew[key].ncit + "\n");
                        }else {
                            outputStream_value.write(resultPropNew[key].project + "," + resultPropNew[key].node_pt + "," + resultPropNew[key].prop  + ",no match,'" + newvalue.v_name + "'," + newvalue.v_n_code + "\n");
                            //console.log('no match', resultPropNew[key].project + "," + resultPropNew[key].node_pt + "," + resultPropNew[key].prop  + ",no match," + newvalue.n + "," +  resultPropNew[key].ncit + "\n");
                        }
                    });
                }
            } else if (resultPropNew[key].type === 'enum') {
                resultPropNew[key].values.forEach(value => {
                    outputStream_value.write(resultPropNew[key].project + "," + resultPropNew[key].node_pt + "," + resultPropNew[key].prop  + ",no match,'" + value.v_name + "'," + value.v_n_code + "\n");
                    //console.log('no match old', resultPropNew[key].project + "," + resultPropNew[key].node_pt + "," + resultPropNew[key].prop  + ",no match," + value.n + "," +  resultPropNew[key].ncit + "\n");
                });
            } else if (resultProp[key].type === 'enum') {
                resultProp[key].values.forEach(value => {
                    outputStream_value.write(resultProp[key].project + "," + resultProp[key].node_pt + "," + resultProp[key].prop + ",'" + value.v_name + "',no match," +  value.v_n_code + "\n");
                    //console.log('no match new', resultProp[key].project + "," + resultProp[key].node_pt + "," + resultProp[key].prop  + value.n + ",no match," +  resultProp[key].ncit + "\n")
                });
            }
        } else {
