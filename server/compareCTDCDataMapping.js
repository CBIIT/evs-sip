...
//const CTDCdata = require('./dataprocess/CTDCdata');
...
const readCTDCyamlFilesNew = () => {
    let jsonData = {};
    let result = undefined;
    let mpJson = yaml.load(dataFilesDir + "/CTDC/ctdc_model_properties_file_new.yaml");
    jsonData.mpData = mpJson;
    let mJson = yaml.load(dataFilesDir + "/CTDC/ctdc_model_file_new.yaml");
    jsonData.mData = mJson;
...
const build_ctdc_node = (dict) => {
    ...
    for (let node_name in dict) {
        ...
        // check existing ctdc_mapping
        if (ctdc_mapping[node_name]) {
            n.ncit = ctdc_mapping[node_name].n_n_code;
            n.ncit_PT = ctdc_mapping[node_name].n_PT;
        }
...
const compareCTDCProp = () => {

    const ctdcdata = readCTDCyamlFiles();
    const ctdcnewdata = readCTDCyamlFilesNew();

    let resultNode = build_ctdc_node(ctdcdata);
    let resultNodeNew = build_ctdc_node(ctdcnewdata);

    let resultProp = build_ctdc_prop(ctdcdata);
    let resultPropNew = build_ctdc_prop(ctdcnewdata);

    const output_file_ctdcnode = path.join(__dirname, 'data_files', 'ctdc_node_mapping_comp.csv');

    const outputStream_node = fs.createWriteStream(output_file_ctdcnode, { flags: 'a' });
    ...
    const output_file_ctdcprop = path.join(__dirname, 'data_files', 'ctdc_prop_mapping_comp.csv');

    const outputStream_prop = fs.createWriteStream(output_file_ctdcprop, { flags: 'a' });
    ...
    const output_file_ctdc_value = path.join(__dirname, 'data_files', 'ctdc_value_mapping_comp.csv');

    const outputStream_value = fs.createWriteStream(output_file_ctdc_value, { flags: 'a' });
    ...
    for (let [key, value] of Object.entries(dcMData.Nodes)) {
        ...
        if (dcMData.Nodes[key].Props != null) {
            for (var i = 0; i < dcMData.Nodes[key].Props.length; i++) {
                //console.log(ctdcMData.Nodes[key].Props[i]);
        ...
        for (var propertyName in dcMData.Relationships) {
            ...
            //console.log(ctdcMData.Relationships[propertyName]);
            //console.log(ctdcMData.Relationships[propertyName].Ends);
